...
router = APIRouter(prefix="/api/agentic-platform", tags=["agentic-platform"])
...
async def catalog():
    return {
        "product": "GOLD-e AI Agentic Business Platform",
        "model": "one_platform_multi_business",
...
async def setup_agent(body: AgentSetupIn, user: dict = Depends(get_current_user)):
    ...
    if workspace.get("plan") != "Pro" or workspace.get("subscription_status") != "active":
        raise HTTPException(
            status_code=402,
            detail={
                "code": "agentic_plan_required",
                "message": "Activate a Pro plan before deploying an Agentic AI business bot.",
                "required_plan": "Pro",
                "pricing_url": "/pricing",
            },
        )
    ...
    doc = {
        ...
        "updated_at": now_iso(),
        "configured_by": user.get("id"),
    }
    await db.agentic_bots.update_one(
        {"workspace_id": user["workspace_id"]},
        {"$set": doc},
        upsert=True,
    )
