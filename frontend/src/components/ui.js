...
const CONTROL_BASE = "rounded-[14px] border border-violet-100 bg-white/85 text-slate-900 outline-none transition-all duration-200 placeholder:text-slate-400 focus:border-violet-300 focus:ring-4 focus:ring-violet-100/70";

export function Button({ variant = "primary", size = "md", className = "", children, ...props }) {
  const sizes = { sm: "px-3 py-1.5 text-xs", md: "px-4 py-2.5 text-sm", lg: "px-5 py-3 text-base" };
  const variants = {
    primary: "ge-btn-gloss-primary",
    dark: "bg-slate-950 text-white shadow-sm hover:bg-slate-800",
    outline: "ge-btn-gloss-secondary",
...
export function Input({ className = "", label, ...props }) {
  return (
    <label className="block">
      ...
      <input className={`w-full px-3.5 py-2.5 text-sm ${CONTROL_BASE} ${className}`} {...props} />
    </label>
  );
}
...
export function Textarea({ className = "", label, ...props }) {
  return (
    <label className="block">
      {label && <span className="mb-1.5 block text-xs font-bold text-slate-500">{label}</span>}
      <textarea className={`w-full px-3.5 py-2.5 text-sm ${CONTROL_BASE} ${className}`} {...props} />
    </label>
  );
}
...
export function Select({ className = "", label, children, ...props }) {
  return (
    <label className="block">
      {label && <span className="mb-1.5 block text-xs font-bold text-slate-500">{label}</span>}
      <select className={`w-full px-3.5 py-2.5 text-sm ${CONTROL_BASE} ${className}`} {...props}>{children}</select>
    </label>
  );
}
